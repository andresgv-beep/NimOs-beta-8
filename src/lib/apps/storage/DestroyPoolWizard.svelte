<script>
  ...
  async function submitDestroy() {
    ...
    try {
      ...
      errorMsg = err.message || 'Error al destruir el pool';
      processing = false;
    }
  }
...
>
  ...
  {#if step === 1}
    ...
    <div class="info-grid">
      ...
      {#if pool?.zpoolName && pool.zpoolName !== pool.name}
        <div class="info-row">
          <div class="info-label">zpool</div>
          <div class="info-value mono">{pool.zpoolName}</div>
        </div>
      {/if}
      <div class="info-row">
        <div class="info-label">Sistema</div>
        <div class="info-value">{(pool?.type || 'zfs').toUpperCase()} · {pool?.vdevType || 'single'}</div>
      </div>
      <div class="info-row">
        <div class="info-label">Capacidad</div>
        <div class="info-value">{pool?.size || fmtBytes(pool?.total) || '—'}</div>
      </div>
      <div class="info-row">
        <div class="info-label">Estado</div>
        <div class="info-value">
          <LED size={7} variant={pool?.health === 'ONLINE' ? 'ok' : 'warn'} />
          <span>{pool?.health || 'exported'}</span>
        </div>
      </div>
      <div class="info-row">
        <div class="info-label">Mount point</div>
        <div class="info-value mono sm">{pool?.mountPoint || '—'}</div>
      </div>
    </div>
