...
- Wipe de discos (con guard de `.nimbus-pool.json` si existe en mountpoint candidato)
- `mkfs.btrfs` con profile elegido
- Mount en `/nimbus/pools/<name>`
- Crear `.nimbus-pool.json` en root del filesystem
...
- Leer/escribir `.nimbus-pool.json` si no existe
- Cambio de metadata
- Operación tipo `control_state_change` en histórico
...
- Si `control_state = managed` y no existe `.nimbus-pool.json` en el mountpoint → es un estado inconsistente → marcar como `recovery` y notificar al usuario (Beta 9)
