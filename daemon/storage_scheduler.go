...
// al boot, y permite pararlo con StopStorageScheduler (útil en tests, en
// shutdown limpio del daemon, o si se quiere desactivar el background).
//
...
// StopStorageScheduler para el reconciler global. Bloquea hasta que el
// ciclo en curso (si lo hay) termine. Idempotente.
func StopStorageScheduler() {
	globalReconcilerMu.Lock()
	r := globalReconciler
	globalReconciler = nil
	globalReconcilerMu.Unlock()

	if r == nil {
		return
	}
	r.Stop()
	logMsg("Storage scheduler stopped")
}
