...
type Pool struct {
	...
	Name          string       // Nombre legible, único, puede cambiar (rename)
	BtrfsUUID     string       // UUID del filesystem (de blkid)
	Profile       Profile      // single | raid1 | raid1c3 | raid10
	MountPoint    string       // /nimbus/pools/<name>
	Role          Role         // Beta 8 siempre "data". Consumers futuros.
	Compression   string       // none | lzo | zstd:1..15
	ControlState  ControlState // managed | observed (Beta 8)
	DiscoveredAt  *time.Time   // primera vez que se vio, nullable
	CreatedAt     time.Time
	...
	Devices      []Device // miembros del pool
}
...
type Device struct {
	...
	Serial      string    // IDENTIDAD ABSOLUTA
	ByIDPath    string    // /dev/disk/by-id/ata-...
...
type Operation struct {
	ID          string          // UUID
	Type        OperationType   // create_pool | rename_pool | ...
	PoolID      *string         // nullable
	Status      OperationStatus // pending | in_progress | completed | failed | rolled_back | cancelled
	StartedAt   time.Time
