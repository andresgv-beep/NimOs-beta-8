...
func TestStorageRepoPoolCRUD(t *testing.T) {
	...
	pool := &Pool{
		...
		MountPoint: "/nimbus/pools/multimedia",
	}
...
func TestStorageRepoPoolUniqueConstraints(t *testing.T) {
	...
	p1 := &Pool{
		...
		Profile: ProfileSingle, MountPoint: "/nimbus/pools/data",
	}
	withTx(t, conn, func(tx *sql.Tx) error {
	...
	p2 := &Pool{
		...
		Profile: ProfileSingle, MountPoint: "/nimbus/pools/data2",
	}
	...
	p3 := &Pool{
		...
		Profile: ProfileSingle, MountPoint: "/nimbus/pools/other",
	}
	tx, _ = conn.BeginTx(ctx, nil)
...
func TestStorageRepoListPoolsByControlState(t *testing.T) {
	...
	pools := []*Pool{
		{ID: "p1", Name: "a", BtrfsUUID: "u1", Profile: ProfileSingle,
			MountPoint: "/nimbus/pools/a", ControlState: ControlStateManaged},
		{ID: "p2", Name: "b", BtrfsUUID: "u2", Profile: ProfileRaid1,
			MountPoint: "/nimbus/pools/b", ControlState: ControlStateObserved},
		{ID: "p3", Name: "c", BtrfsUUID: "u3", Profile: ProfileRaid1,
			MountPoint: "/nimbus/pools/c", ControlState: ControlStateManaged},
	}
	for _, p := range pools {
		withTx(t, conn, func(tx *sql.Tx) error {
